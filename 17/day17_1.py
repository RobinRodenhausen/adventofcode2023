...
def neighbors(
    ...
    if streak < 3 and 0 <= line + d_line < len(blocks) and 0 <= column + d_column < len(blocks[0]):
    ...
    for d_line, d_column in left, right:
        # bounds check
        if 0 <= line + d_line < (len(blocks)) and 0 <= column + d_column < len(blocks[0]):
